...
import { PerspectiveProvider } from "@/components/fairy/PerspectiveProvider";
...
describe("WorkspaceTabs — per-partner restricted views (Req 1.2)", () => {
  it("shows only His and Together when signed in as Daniel (him)", () => {
    window.localStorage.setItem("fairy.perspective", "him");
    renderWorkspace();
  ...
  it("shows only Her and Together when signed in as Maya (her)", () => {
    window.localStorage.setItem("fairy.perspective", "her");
    renderWorkspace();
