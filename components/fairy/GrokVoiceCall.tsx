...
// ===========================================================================
// Live Grok Voice call console (components/fairy/GrokVoiceCall.tsx)
//
...
// the Fairy agent — no scripted responder, 100% xAI Grok Voice.
//
...
//   • session.update sets the Fairy system prompt, voice, server_vad turn
  ...
  const startCall = useCallback(async () => {
    ...
    try {
      ...
      ws.onopen = () => {
        ws.send(
          JSON.stringify({
            ...
            session: {
              ...
              instructions:
                systemPrompt ??
                "You are Fairy, an authorized assistant verifying fertility benefits on behalf of a couple. Ask your questions one at a time and confirm answers.",
        ...
        // Let Grok open the call (it speaks first, as the Fairy agent).
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 16, maxWidth: 390, margin: "0 auto", padding: 16 }}>
      ...
      <p style={{ fontSize: 13, color: "#555", margin: 0 }}>
        Speak as the {callType === "insurance" ? "insurance rep" : "clinic scheduler"}. Fairy
          ...
          turns.map((t, i) => (
            ...
            >
              <strong style={{ fontSize: 11, color: "#777", display: "block" }}>
                {t.speaker === "agent" ? "Fairy (Grok)" : "You (rep)"}
              </strong>
              {t.text}
            </div>
          ))
