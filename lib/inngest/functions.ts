...
export const fairyIntakeWorkflow = inngest.createFunction(
  { id: "fairy-intake-workflow", name: "Fairy intake → reactive graph" },
  ...
  async ({ event, step }) => {
    ...
    const { run } = await runFairyWorkflow(coupleId, {
      runStep,
...
export const reactiveSummaryFunction = inngest.createFunction(
  { id: "fairy-reactive-summary", name: "Fairy reactive Doctor_Summary refresh" },
...
export const functions = [fairyIntakeWorkflow, reactiveSummaryFunction];
