// ===========================================================================
// Property test — Feature: fairy, Property 22: Approval resume is idempotent
...
// always returns { approved: true } and re-running runFairyWorkflow N times
...
describe("Feature: fairy, Property 22: Approval resume is idempotent and never double-books", () => {
    ...
    await fc.assert(
      fc.asyncProperty(fc.integer({ min: 1, max: 5 }), async (deliveries) => {
        ...
        for (let i = 0; i < deliveries; i++) {
          results.push(await runFairyWorkflow(COUPLE_ID, h.deps));
        }
  ...
  test("a single in-process run that re-finalizes is also idempotent (in-run guard)", async () => {
    ...
    const first = await runFairyWorkflow(COUPLE_ID, fullDeps);
    const second = await runFairyWorkflow(COUPLE_ID, fullDeps);
