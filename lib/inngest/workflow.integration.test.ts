...
//   Workflow orchestration for the Fairy EVENT-DRIVEN REACTIVE GRAPH.
//
// With the Grok/Voice agent fully MOCKED (no network, no Inngest server), this
// asserts the reactive-graph guarantees of runFairyWorkflow:
//
...
describe("reactive graph — idempotent finalize (Req 17.3)", () => {
  it("repeated approvals / finalize calls produce exactly one booking + one Jun 25 event", async () => {
    ...
    const first = await runFairyWorkflow("couple_001", h.deps);
    ...
    const second = await runFairyWorkflow("couple_001", h.deps);
