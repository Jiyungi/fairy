// ===========================================================================
// Fairy reactive-graph workflow — testable core (lib/inngest/workflow.ts)
//   — Req 7, 17, 18, 19
//
...
export async function stepGenerateTasks(
  ...
  await deps.saveTasks(ctx.coupleId, tasks);
  ctx.tasks = tasks;
  return tasks;
}
 ...
 * and a FairyWorkflowError is thrown to halt the pipeline (Req 7.6).
 */
 ...
 * first failed branch, dependents stay `pending`, and a FairyWorkflowError is
...
async function executeParallel<A, B>(
  ...
  if (firstError) {
    ...
    await deps.saveWorkflowRun(run);
    throw new FairyWorkflowError(fe.step, fe.name, fe.message);
  }
...
/**
 * Run the full Fairy reactive graph for a couple (Req 7.1). Two branch-pairs
