...
//
//   Strategy: inject a custom `runStep` (StepRunner) into runFairyWorkflow that
...
function buildHarness(delays: Record<ParallelStep, number>): Harness {
  ...
  // defaultWorkflowDeps() is merged inside runFairyWorkflow; reference it here
...
describe("Feature: fairy, Property 21: Parallel branches both complete before the workflow proceeds past a fan-in", () => {
    ...
    await fc.assert(
      fc.asyncProperty(
        ...
        async (delays) => {
          ...
          const { run } = await runFairyWorkflow(COUPLE_ID, deps);
