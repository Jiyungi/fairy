// ===========================================================================
// Property test — Feature: fairy, Property 24: Reactive summary fires on every
//   call.completed.
//
...
//
// Strategy: drive runFairyWorkflow with a fully-mocked agent (no network) over
...
describe("Feature: fairy, Property 24: Reactive summary fires on every call.completed", () => {
  test("each completed call emits call.completed once and refreshes a grounded summary", async () => {
    await fc.assert(
      ...
      fc.asyncProperty(fc.constant(COUPLE_ID), async (coupleId) => {
        ...
        const { run } = await runFairyWorkflow(coupleId, h.deps);
